...
| RD-Correct | Realinhamento à IA canônica (4 abas) + trim cópia literal | ✅ **Concluído (2026-05-14)** — commit `f7c3045`, typecheck baseline (14, 0 novos) | @dev |
| RD-3 | Diário (DiaryB) + Perfil/Eu (ProfileB visual) | ✅ **Concluído (2026-05-14)** — commit `d5669af`, +609/-142, mood picker + FAB no Diário, hero gradient + stats + menu list expansível no Perfil, entry point Diário via `router.push('/diario')` | @dev |
| RD-4 | Onboarding + Login + Paywall | ✅ **Concluído (2026-05-14)** — commit `6fda949`, typecheck baseline (14, 0 novos), EAS Build APK `ae02e868` FINISHED | @dev → @devops |
| RD-5 | Plano de Parto + Enxoval | ✅ **Concluído (2026-05-14)** — commit `216c047` (+813), typecheck baseline (14, 0 novos), fix crash `074f4fa`, EAS Build APK `3e3c8ab2` FINISHED | @dev → @devops |
| RD-6 | Consultas + Vitaminas + Exames (KickCounter mantido em ferramentas.tsx) | ✅ **Concluído (2026-05-14)** — commit `d320e18` (+1354), typecheck baseline (14, 0 novos), EAS Build APK `3121dcc3` FINISHED | @dev → @devops |
| RD-7 | Plus (Álbum + Artigo + Chat) | ✅ **Concluído (2026-05-15)** — commit `5061811` (+1078), typecheck baseline (14, 0 novos), EAS Build APK `7f129dea` FINISHED **mas crashou no boot em celular fresco** — fix em FX-1. | @dev → @devops |
| FX-1 | Fix crash APK 7f129dea — guard getProfile + onboarding null safety + lazy notif handler + ErrorBoundary | ✅ **Concluído (2026-05-15)** — commit `751d00c` (+109/-14), typecheck 14→12 erros, EAS Build APK `bc35b42c-e0c8-47ea-b597-cdbed92d20f4` IN_QUEUE. Causa raiz tripla diagnosticada por @qa-investigator + @devops-investigator + @architect-investigator em paralelo. | @dev → @qa → @devops |
| FX-2 | Sprint Responsividade & Safe-Area — corrigir overflow de texto + botões atrás da tab bar + scroll insuficiente no Redmi Note 9 (MIUI, 19.5:9) | ✅ **Concluído (2026-05-15)** — 14 arquivos + hook `useBottomSpacing`. SafeAreaProvider raiz, maxFontSizeMultiplier=1.3, folga inferior dinâmica em todas as telas. typecheck: 0 novos erros. | @dev → @qa |
...
| UR-01-PLAN | Plano Onda 1 pré-G-7 + régua editorial de avisos (12 categorias + 8 sinais de alerta, FEBRASGO/MS/ANVISA/ACOG validadas) | ✅ **Concluído (2026-05-20)** — planejamento puro, sem código. Plano canônico: `C:\Users\USUARIO\.claude\plans\ok-atualize-o-plano-whimsical-hedgehog.md` | GESTOR |
| UR-01-REORDER | Replanejamento Onda 1 — funcional antes de editorial. Confirmação dos 3 primeiros itens, UR-S4/S6 ao final, criação spec Sessão E-1 (Enxoval Completo pós-G-7), UR-S5 reduzida a MVP-MVP | ✅ **Concluído (2026-05-21)** — planejamento puro, sem código, sem commit. Nova ordem: UR-S3 → UR-S5 → UR-S1 → UR-S2 → (UR-S6 + UR-S4). Spec E-1: `docs/plans/pendentes/enxoval-completo-spec.md` | GESTOR |
| UR-S3-PLAN | Plano detalhado UR-S3 — decisões: FeedSnap embutido no Início + Explorar vira hub Biblioteca Plus (3 cards → album/article/chat root) | ✅ **Concluído (2026-05-21)** — plano em `docs/plans/pendentes/ur-s3-reorganizacao-inicio-explorar.md`, sem código | GESTOR |
| UR-S3 | Reorganização Início ↔ Explorar — refator dashboard ScrollView→FlatList+ListHeader + reescrita explorar como hub Plus | ⏳ **Próxima execução** — plano fechado, aguarda @po | @po → @dev → @qa → @devops |
| UR-S5 | Template enxoval mínimo — `src/data/enxovalTemplate.ts` com 30 itens em memória na 1ª abertura, 4 categorias atuais | ⏳ Planejada (escopo mínimo) — feature completa virou Sessão E-1 pós-G-7 | @dev |
...
| UR-01-PLAN | analisar feedback + benchmark e quebrar 🔴 Alta em stories antes de G-7 | ✅ **Concluído (2026-05-20)** — plano canônico em `C:\Users\USUARIO\.claude\plans\ok-atualize-o-plano-whimsical-hedgehog.md` + régua editorial `docs/user-research/2026-05-20-regua-editorial-avisos-gestacao.md` + HTML checklist. Onda 1 = 6 stories (UR-S1..S6). Aguarda revisão UR-S3 antes de execução. |
| UR-S1..S6 | Onda 1 — passada editorial 40 semanas (avisos+sinais+nutrição+tom carinhoso) + glossário inline completo + reorganização Início↔Explorar + template enxoval | ⏳ **Próxima execução** — split em Sub-A (editorial: S1+S4+S6) e Sub-B (componentes: S2+S3+S5). G-7 mantido 01/jun. |
